...
class NASAPowerHumidityPreprocessor:
    ...
    def create_basic_visualization(self, output_dir="nasa_hum_plots", save_plots=True):
        ...
        if not valid_data.empty:
            ...
            plt.ylabel('Kelembaban Relatif (%)', fontsize=12)
            plt.xlabel('Tahun', fontsize=12)
            plt.title('NASA POWER: Relative Humidity at 2 Meters (RH2M) Time Series', fontsize=13, fontweight='bold')
        ...
        monthly_data = []
        for month in months:
            month_hum = self.data[self.data['month'] == month]['RH2M'].dropna()
            monthly_data.append(month_hum if not month_hum.empty else pd.Series(dtype=float))
        ...
        plt.title('Distribusi Kelembaban Bulanan NASA POWER (2005-2025)', fontsize=13, fontweight='bold')
        plt.grid(True, alpha=0.3)
        ...
        if len(decomp_data) >= 730:  # Minimal 2 tahun data
            try:
                ...
                axes[0].set_title('Dekomposisi Deret Waktu Kelembaban NASA POWER', 
