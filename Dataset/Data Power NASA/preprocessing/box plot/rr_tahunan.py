...
data_by_year = [df[df['Year'] == year]['RR_imputed'].values for year in years]
...
ax.set_ylabel('Daily Rainfall (mm)', fontsize=12, fontweight='bold')
ax.set_title('Annual Daily Rainfall Distribution (2005-2025)', fontsize=14, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('annual_daily_rainfall_boxplot.png', dpi=300, bbox_inches='tight')
plt.show()

# Print summary statistics
print("\n=== Annual Daily Rainfall Statistics (mm) ===\n")
for year in years:
    data = df[df['Year'] == year]['RR_imputed']
