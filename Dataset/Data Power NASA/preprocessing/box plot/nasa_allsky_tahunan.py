...
# Read the CSV file
df = pd.read_csv('nasa-box-plot.csv')
...
data_by_year = [df[df['Year'] == year]['ALLSKY_SFC_SW_DWN'].values for year in years]
...
ax.set_ylabel('Daily Solar Radiation (MJ/m²/day)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('annual_daily_solar_radiation_boxplot.png', dpi=300, bbox_inches='tight')
plt.show()

# Print summary statistics
print("\n=== Annual Daily Solar Radiation Statistics (MJ/m²/day) ===\n")
for year in years:
    data = df[df['Year'] == year]['ALLSKY_SFC_SW_DWN']
